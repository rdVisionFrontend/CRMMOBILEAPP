...
const Ticket = () => {
    ...
    }, [statusmodal, emailModal, dispatch, user]);
    ...
    const fetchTicket = async () => {
        try {
          ...
          const response = await apiInstance.get('/third_party_api/ticket/getAllNewTickets');
    ...
    return (
        <>
                    ...
                    {ticketData && ticketData.length > 0 ? (ticketData?.map((item, index) => (
                        <View key={index} style={styles.item}>
                            ...
                            >
                                ...
                                {readTicket && readTicket.some((ele) => ele === item.uniqueQueryId) && (
                                    <Image
                                ...
                                >
                                    <View style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'flex-start' }}>
                                        ...
                                        <View style={{ display: 'flex', flexDirection: 'row', gap: 4 }}>
                                            <Text onPress={() => toggleAccordion(item.uniqueQueryId)} style={styles.headerText}>{item.senderName}</Text>
                                            <CountryFlagTable key={index} flagName={item.senderCountryIso} />
                                        </View>
                                        <TouchableOpacity onPress={() => toggleModal(item.queryMessage)}>
                                            <Text style={styles.text}>{item.queryMessage && item.queryMessage.slice(0, 20)}</Text>
                                        </TouchableOpacity>
                                    </View>

                                    {/* Main item which toggles the dropdown menu */}
                                    <TouchableOpacity onPress={() => openStatusModal(item)}>
                                        <Text style={[styles.dropdownButton, { backgroundColor: getStatusColor(selectedStatus || 'New') }]}>
                                            {selectedStatus ? ` ${selectedStatus}` : `${item.ticketstatus}`}
                                        </Text>
                                    </TouchableOpacity>

                                    <View style={{ display: 'flex', gap: 5, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                                        {/* WhatsApp */}
                            ...
                            {expanded === item.uniqueQueryId && (
                                <View style={styles.content}>
                                    ...
                                    >
                                        ...
                                        <View style={{ display: 'flex', flexDirection: 'row', gap: 5, justifyContent: 'flex-start', alignItems: 'center' }}>

                                            <Image source={{ uri: 'https://cdn-icons-png.flaticon.com/128/3059/3059561.png' }} style={{ height: 12, width: 12 }} />
                                            <Text style={styles.contentText}>{` ${formatMobile(item.senderMobile)}`}</Text>
                                        </View>
                                        ...
                                        <View style={{ display: 'flex', flexDirection: 'row', gap: 5, justifyContent: 'flex-start', alignItems: 'center' }}>
                                            ...
                                            />
                                            <Text style={styles.contentText}>{`${formateEmail(item.senderEmail)}`}</Text>
                                    ...
                                    <Text style={styles.contentText}>{item.subject && item.subject.slice(16)}</Text>
                                </View>
                            )}
            ...
            {/* Status Modal */}
            <View style={styles.emailModal}>
                {statusmodal && <StatustModal data={statusData} closeModal={closeEmailModal} />}
            </View>
        </>
    );
};
